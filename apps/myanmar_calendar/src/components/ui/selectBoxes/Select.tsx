...
>(({ className, children, config: {} = {}, ...props }, ref) => {
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        "border flex-shrink-0 border-cgray-300 rounded-sm flex justify-stretch items-center w-full px-2 outline-0 hover:bg-cgray-100 hover:border-cgray-300 active:border-cgray-500 h-cgray-50-md",
        className
      )}
      {...props}>
      {children}
  ...
  ({ className, children, position = "popper", align = "start", ...props }, ref) => (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ref={ref}
        className={cn("bg-cgray-0 rounded-md border border-cgray-200 mt-1 shadow-lg z-[10]", className)}
        position={position}
        align={align}
        {...props}>
...
>(({ className, children, showIndicator = false, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "flex items-center h-[2.2rem] rounded cursor-pointer hover:bg-cgray-100 focus:bg-cgray-100 active:bg-cgray-100 px-2 capitalize outline-0 data-[state=checked]:bg-cgray-200 w-full text-[0.9rem] text-cgray-600",
