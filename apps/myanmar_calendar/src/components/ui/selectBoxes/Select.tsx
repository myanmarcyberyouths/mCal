...
>(({ className, children, config: {} = {}, ...props }, ref) => {
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        "border flex-shrink-0 border-gray-200 rounded-sm flex justify-stretch items-center w-full px-2 outline-0 hover:bg-gray-100 hover:border-gray-300 active:border-gray-500 h-input-md",
        className,
      )}
      {...props}
    >
  ...
  (
    ...
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ref={ref}
        className={cn(
          "bg-gray-0 rounded-md border border-gray-200 mt-1 shadow-lg z-[10]",
          className,
        )}
        position={position}
        align={align}
...
>(({ className, children, showIndicator = false, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "flex items-center h-[2.2rem] rounded cursor-pointer hover:bg-gray-50 focus:bg-gray-100 active:bg-gray-100 px-2 capitalize outline-0 data-[state=checked]:bg-gray-200/50 w-full text-[0.9rem] text-gray-600",
